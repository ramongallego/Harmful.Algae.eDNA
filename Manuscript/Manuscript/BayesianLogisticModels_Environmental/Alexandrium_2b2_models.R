...
arm.fit14 <- stan_glmer(presence ~ SalinityStd + (0 +  pHStd | Season),
                        ...
                        iter = 1000,
                        chains = 4)
arm.fit15 <- stan_glmer(presence ~ 0 + pHStd + (1 + TempStd | Season),
                       data = mydataset,
                       family = "binomial",
                       prior_intercept = normal(0, 1),
                       prior = normal(0,1),
                       iter = 1000,
                       chains = 4)
arm.fit16 <- stan_glmer(presence ~ pHStd + (0 + TempStd | Season),
                       data = mydataset,
                       family = "binomial",
                       prior_intercept = normal(0, 1),
                       prior = normal(0,1),
...
saveRDS(arm.fit15, file = "BayesianLogisticModels_Environmental/Alexandrium_2b2_BestModel.RDS")
