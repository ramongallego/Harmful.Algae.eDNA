...
arm.fit14 <- stan_glmer(presence ~ SalinityStd + (0 +  pHStd | Season),
                        ...
                        iter = 1000,
                        chains = 4)
arm.fit15 <- stan_glmer(presence ~ 0 + pHStd + (1 + SalinityStd | Season),
                        data = mydataset,
                        family = "binomial",
                        prior_intercept = normal(0, 1),
                        prior = normal(0,1),
...
#model compare
modList <- paste0("arm.fit", 1:15)
names(modList) <- paste0("arm.fit", 1:15)
modList %>% 
  map(as.symbol) %>% 
  map(eval) %>% 
  map(waic) %>% 
...
mydataset %>% 
  add_fitted_draws(arm.fit13, n = 1000) %>% 
  ggplot(aes(x = Salinity, y = presence, color = Season)) +
  geom_point() +
  facet_grid(~ Season, scales = "free_x") +
  stat_lineribbon(aes(y = .value), .width = c(.95, .5)) +
  scale_fill_brewer()

#saveRDS(arm.fit13, file = "BayesianLogisticModels_Environmental/Karlodinium_a27_BestModel.RDS")
